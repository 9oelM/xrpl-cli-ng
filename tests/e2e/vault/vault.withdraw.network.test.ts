...
beforeAll(async () => {
  ...
  // Create one XRP vault to use across all withdraw tests
  const result = runCLI([
    "--node", "devnet",
    "vault", "create",
    "--asset", "0",
...
describe("vault withdraw (devnet)", () => {
  it("deposits XRP then withdraws, outputs Vault ID and tesSUCCESS", () => {
    const deposit = runCLI([
      "--node", "devnet",
      "vault", "deposit",
      "--vault-id", vaultId,
      "--amount", "2",
    ...
    expect(deposit.status, `deposit: ${deposit.stderr}`).toBe(0);
  ...
  it("--destination sends redeemed assets to a different account", async () => {
    ...
    expect(
      (fundTx.result.meta as { TransactionResult?: string } | undefined)?.TransactionResult
    ).toBe("tesSUCCESS");
  ...
  it("--dry-run prints VaultWithdraw tx JSON without submitting", () => {
    const result = runCLI([
      "--node", "devnet",
      "vault", "withdraw",
      "--vault-id", vaultId,
  ...
  it("--account + --keystore + --password key material withdraws successfully", () => {

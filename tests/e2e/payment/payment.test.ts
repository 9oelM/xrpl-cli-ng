...
import type { TrustSet, Payment as XrplPayment, MPTokenIssuanceCreate, MPTokenAuthorize } from "xrpl";
...
describe("path payment flags", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      // Fund flagHolder with XRP from flagIssuer
      const fundTx = await client.autofill({
        TransactionType: "Payment",
      ...
      const signedFund = flagIssuer.sign(fundTx);
      await client.submitAndWait(signedFund.tx_blob);

      // flagHolder sets trust line for flagIssuer/USD
      ...
      const signedTrust = flagHolder.sign(trustTx);
  ...
  it("--partial --json output includes deliveredAmount (IOU partial payment)", () => {
    // Use IOU partial payment — XRP-to-XRP partial is not allowed (temBAD_SEND_XRP_PARTIAL)
    const result = runCLI([
      "--node", "testnet",
      "payment",
